...
func SaveDiskDirInfo(uid, pid int64, phone, dirName string) (bool, int64) {
	...
	//filetype 文件夹1 文件-1
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(),"start")
	exec, error := stmt.Exec(uid, pid, phone, dirName, 1, 1)
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(),saveUDiskFileDirinfo,uid, pid, phone, dirName, 1, 1)
	if error != nil {
		fmt.Println(tAG_userDiskfile, "failed to Exec error:", error)
		return false, -1
	}
	...
	if rows, error := exec.RowsAffected(); error == nil {
		if rows <= 0 {
			//执行成功，但未添加，
			fmt.Printf(tAG_userDiskfile, "failed with hash has been upload", error)
			return false, -1
		}
	}
...
func SaveDiskFileInfo(uid, pid int64, phone, filehash, filename, location string, filesize int64, minitype string, ftype int, video_duration string) bool {
	...
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), saveUDiskFileinfo,uid, pid, phone, filehash, filename, filesize, location, 1, minitype, ftype, video_duration)
	if error != nil {
		fmt.Println(tAG_userDiskfile, "failed to Exec error:", error)
		return false
	}
	if rows, error := exec.RowsAffected(); error == nil {
		if rows <= 0 {
			//执行成功，但未添加，
			fmt.Printf(tAG_userDiskfile, "failed with hash:%s has been upload", error)
			return false
		}
	}
	return true
...
func UpdateUserDiskInfoStatusBySha1(filehash string, filestatus int8) bool {
	...
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), updateUDiskFileInfo)
	if error != nil {
...
func GetUserDiskInfoById(id int64) (*TableUserFile, error) {
	...
	defer stmt.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileByid)
	tfile := TableUserFile{}
	...
	if error != nil {
		fmt.Println(tAG_userDiskfile, "failed to QueryRow error:", error)
		return nil, error
	}
	return &tfile, nil
}
...
func GetUserDiskMetaByPidUidSha1(filehash string, uid, pid int64) (*TableUserFile, error) {
	...
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileByUidAndPidAndSha1,filehash, uid, pid)
	error = row.Scan(
		&tfile.Id, &tfile.PId, &tfile.Uid, &tfile.Phone, &tfile.FileHash, &tfile.FileHash_Pre, &tfile.FileName, &tfile.FileSize, &tfile.FileLocation, &tfile.Create_at, &tfile.Update_at, &tfile.Filetype, &tfile.Minitype, &tfile.Ftype, &tfile.Video_duration)
	if error != nil {
		fmt.Println(tAG_userDiskfile, "failed to QueryRow error:", error)
...
func GetUserDiskInfoByUidSha1(filehash string, uid int64) (*TableUserFile, error) {
	...
	defer stmt.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(),selectUDiskFileBySha1)
	tfile := TableUserFile{}
...
func GetUserDiskListByUidPidDirName(uid, pid int64, filename string) (tableUserFile []TableUserFile, err error) {
	...
	defer  rowdata.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileByUidAndPidAndFileName, uid, pid, filename)
	if error != nil {
...
func GetUserDiskFileListByUidPid(uid, pid int64, pageNo, pageSize, lastid int64) (tableUserFile []TableUserFile, err error, total int64) {
	...
	defer  rowdata.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileByUidAndPidPage, uid, pid, lastid, pageSize)
	if error != nil {
		fmt.Println("failed to Exec error:", error)
		return tableUserFile, error, 0
	}
...
func GetUserDiskListCountByUidPid(uid, pid int64, lastid int64) (count int64) {
	...
	defer result.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileCountByUidPid, pid, uid)
	if err != nil {
		return 0
	}
...
func GetUserDiskListMaxCountByUid(uid, pid int64) (maxid int64) {
	...
	result, err := stmt.Query(pid, uid)
	defer  result.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(),selectMaxIdFromUserDiskFile, pid, uid)
	if err != nil {
		return 0
	}
	var countResult sql.NullInt64
...
func GetUserDiskListCountByUid(uid, lastid int64) (count int64) {
	...
	defer result.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileCountByUid, uid)
	if err != nil {
		return 0
	}
	var countResult sql.NullInt64
...
func GetUserDiskFileListMetaByUid(uid int64, pageNo, pageSize, lastid int64) (tableUserFile []TableUserFile, err error, total int64) {
	...
	defer rowdata.Close()
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(), selectUDiskFileByUidPage, uid, lastid, pageSize)
	if error != nil {
		fmt.Println("failed to Exec error:", error)
		return tableUserFile, error, 0
	}
...
func UpdateUserDiskFileStatusBySha1sUidPid(uid, pid int64, filestatus int8, sha1s []string) bool {
	...
	fmt.Println(tAG_userDiskfile, utils.RunFuncName(),sprintf, filestatus, uid, pid)
...
func UpdateUserDiskFileDirPreSha1ById(sha1 string, id int64) bool {
	...
	fmt.Println(tAG_userDiskfile,  utils.RunFuncName(), updateUDiskFileDirfile_sha1_pre, sha1, id)
	if error != nil {
		fmt.Println(tAG_userDiskfile, "failed Exec error:", error)
		return false
	}
